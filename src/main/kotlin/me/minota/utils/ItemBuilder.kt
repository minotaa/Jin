...
class ItemBuilder(material: Material) {
    ...
    fun toLeatherArmor(): ItemBuilder {
        if (item.type == Material.LEATHER_HELMET || item.type == Material.LEATHER_CHESTPLATE || item.type == Material.LEATHER_LEGGINGS || item.type == Material.LEATHER_BOOTS) {
            meta = item.itemMeta as LeatherArmorMeta
        }
        return this
    }
